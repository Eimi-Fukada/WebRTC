...
const makeRequest: MakeRequest = <T>(config: RequestConfig) => {
  return async (requestConfig?: Partial<RequestConfig>) => {
    ...
    try {
      ...
      if (res.result !== HttpStatus.success) {
        const error = { code: res.result, message: res.resultMessage };
        return { err: error, data: remainData, response };
      }
      return {
        err: { code: 0, message: res.resultMessage },
        data: remainData,
        response,
      };
    } catch (err: any) {
